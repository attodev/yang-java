...
import com.airlenet.yang.model.Anydata;
import com.airlenet.yang.model.DataNode;
import com.airlenet.yang.model.InnerNode;
import com.airlenet.yang.model.LeafNode;
import com.airlenet.yang.model.LeafSchemaContext;
import com.airlenet.yang.model.LeafType;
...
import static com.airlenet.yang.runtime.RuntimeHelper.DEFAULT_CAPS;
import static com.airlenet.yang.runtime.RuntimeHelper.PERIOD;
import static com.airlenet.yang.runtime.RuntimeHelper.getCapitalCase;
import static com.airlenet.yang.runtime.impl.YobConstants.E_FAIL_TO_LOAD_CLASS;
...
public class DataTreeBuilderHelper {
    ...
    DataNode.Builder getDataTree(YangSchemaNode curSchema, DataNode.Builder builder,
        ...
        do {
             ...
             * instance node.
             */
            if (curTraversal != PARENT || ModelConverterUtil.isMultiInstanceNode(curNode)) {
                ...
                if (curTraversal == ROOT && !ModelConverterUtil.isAugmentNode(curNode)) {
                    /*
                    ...
                    if (processedObject == null && !ModelConverterUtil.isAugmentNode(curNode)) {
                         ...
                         */
                            ...
                            ModelConverterUtil.isAugmentNode(curNode)) {
                        curTraversal = PARENT;
                        /*
                         ...
                         */
                            ...
                            ModelConverterUtil.isMultiInstanceNode(curNode)) {
                        curTraversal = CHILD;
                    }
                }
            }
             ...
             */
            if (curTraversal == PARENT && ModelConverterUtil.isAugmentNode(curNode)) {
            ...
            if (curTraversal != PARENT && curNode.getChild() != null) {
                ...
                if (ModelConverterUtil.isNodeProcessCompleted(curNode, curTraversal)) {
                    break;
                }
    ...
    private Object processCurSchemaNode(YangNode curNode,
        ...
        if (curNode instanceof YangAugment) {
            ...
            if (augmented instanceof YangCase) {
                ...
                Object obj;
                try {
                    obj = ModelConverterUtil.getAttributeOfObject(
    ...
    Object processAugmentNode(YangNode curNode,
        ...
        Object parentObj = ModelConverterUtil.getParentObjectOfNode(parentNodeInfo,
                                                 curNode.getParent());
        ...
        if (augmented instanceof YangChoice) {
            ...
            try {
                return ModelConverterUtil.getAttributeOfObject(
                        parentNodeInfo.getYangObject(), name);
            } catch (NoSuchMethodException e) {
                throw new ModelConverterException(
                        "Not processable case node with augment in " +
        ...
        Map augmentMap;
        try {
            augmentMap = (Map) ModelConverterUtil.getAugmentObject(parentObj,
                                                AUGMENTATIONS);
             ...
             */
                ...
                Class moduleClass = ModelConverterUtil.getClassLoaderForAugment(curNode, reg);
                if (moduleClass == null) {
                    return null;
                }
    ...
    private Object getAnydataChildObject(YangNode curNode,
        ...
        Anydata parentObj = (Anydata) ModelConverterUtil.getParentObjectOfNode(
        ...
        if (yangNode instanceof YangLeavesHolder) {
            ...
            if (leavesList != null) {
                for (YangLeaf yangLeaf : leavesList) {
                    ...
                    Object parentObj = ModelConverterUtil.getParentObjectOfNode(parentInfo,
                                                             yangNode);
                    Object leafType;
                    try {
                        leafType = ModelConverterUtil.getAttributeOfObject(parentObj,
                                                        ModelConverterUtil.getJavaName(yangLeaf));
                    } catch (NoSuchMethodException e) {
                        throw new ModelConverterException(e);
                    }
                    Object obj = ModelConverterUtil.getLeafObject(yangNode, yangLeaf, parentObj,
                                               leafType, false);
                    if (obj != null) {
    ...
    private List<DataNode.Builder> addToBuilder(
        ...
        List<Object> obj;
        try {
            obj = (List<Object>) ModelConverterUtil.getAttributeOfObject(parentObj,
                                                      ModelConverterUtil.getJavaName(leafList));
        } catch (NoSuchMethodException e) {
            throw new ModelConverterException(e);
        }
        if (obj != null) {
            Set<Object> objects = ModelConverterUtil.getLeafListObject(yangNode, leafList,
                                                    parentObj, obj);
            if (!objects.isEmpty()) {
                Object o = objects.iterator().next();
                if (ModelConverterUtil.isTypeEmpty(leafList.getDataType())) {
                    objects.clear();
