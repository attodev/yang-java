...
import static com.airlenet.yang.compiler.datamodel.YangSchemaNodeType.YANG_CHOICE_NODE;
...
class YobWorkBench {
    ...
    private static void setObjectInParent(Object parentObj, String setter,
        ...
        try {
            ...
            if (setter.equals(YobUtils.ANYDATA_SETTER)) {
            ...
            if (type == MULTI_INSTANCE_NODE) {
                ...
                method = parentClass
                        .getDeclaredMethod(YobConstants.ADD_TO + YobUtils.getCapitalCase(setter), classType);
            } else {
            ...
            log.error(YobConstants.L_FAIL_TO_GET_FIELD, parentClassName);
            throw new ModelConverterException(YobConstants.E_FAIL_TO_GET_FIELD + parentClassName, e);
        } catch (NoSuchMethodException e) {
            log.error(YobConstants.L_FAIL_TO_GET_METHOD, parentClassName);
            throw new ModelConverterException(YobConstants.E_FAIL_TO_GET_METHOD + parentClassName, e);
        } catch (InvocationTargetException | IllegalAccessException e) {
            log.error(YobConstants.L_FAIL_TO_INVOKE_METHOD, parentClassName);
            throw new ModelConverterException(YobConstants.E_FAIL_TO_INVOKE_METHOD + parentClassName, e);
        }
    }
    ...
    static void addInAugmentation(Object builder,
        ...
        try {
            Method method = baseClass.getDeclaredMethod(YobConstants.ADD_AUGMENT_METHOD,
                                                        InnerModelObject.class);
            method.invoke(builder, instance);
        } catch (NoSuchMethodException e) {
            log.error(YobConstants.L_FAIL_TO_GET_METHOD, YobConstants.ADD_AUGMENT_METHOD);
            throw new ModelConverterException(YobConstants.E_FAIL_TO_GET_METHOD + YobConstants.ADD_AUGMENT_METHOD, e);
        } catch (InvocationTargetException | IllegalAccessException e) {
            log.error(YobConstants.L_FAIL_TO_INVOKE_METHOD, YobConstants.ADD_AUGMENT_METHOD);
            throw new ModelConverterException(YobConstants.E_FAIL_TO_INVOKE_METHOD + YobConstants.ADD_AUGMENT_METHOD, e);
        }
    }
